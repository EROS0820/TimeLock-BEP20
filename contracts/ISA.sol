...
contract ISA is Context, IBEP20, Ownable {
    ...
    uint256 public _timeLimit = 10; //10 seconds
    address private charity_address = 0x6Aa14Bc0909d6ef73Ded04a7dc4Fd3A7A0383973;
    ...
    function _transfer(
        ...
        require((recipientBalance + amount) <= (_tTotal / 200), "Recipent balance exceeds limit");
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount, uint256 amount_accounts) private {
        ...
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        _rOwned[charity_address] = _rOwned[charity_address].add(rBurn);
        _tTotal = _tTotal.sub(tLiquidity);
        _buyHistories[recipient].push(History(block.timestamp, tAmount));

        if (amount_accounts != 0) {
            uint256 _sReward = rBurn.div(amount_accounts);
            for (uint256 i = 0; i < _accounts.length; i ++) {
                address _temp = _accounts[i];
                if (_temp != address(0) && _temp != sender && _temp != recipient) {
